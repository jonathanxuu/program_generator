...
// as to each leaf ：
// exec.read_new_leaf exec.read_and_copy exec.multi_rphash loadw.adv( read uuid) rphash （generate each saltedhash） 
...
function auto_program_generater(leaves_number, constraints) {
    for (const key in constraints) {
        if (Object.hasOwnProperty.call(constraints, key)) {
            ...
            if (element.fields.length == 1) {
                constraint_program = handle_single_constraint(leaves_number, element.fields[0], element.operation[0], element.value);
            } else {
...
function handle_single_constraint(
    leaves_number,
    fields,
    operation,
    value
...
function handle_multi_constraint(
    leaves_number,
    fields,
    operation,
    value
    ...
    switch (operation[0]) {
        case "sum": {
            ...
            if (decimal > 1) panic("decimal value has more than 1 digits!")
            if (decimal == 0) {
...
function prepare_auth_path_and_read_to_mem(
    leaves_number,
    field,
    number_to_save
) {
...
function prepare_string_operation(
    operation,
    value
) {
    var program_text;
...
function prepare_number_operation_single(
    operation,
    value
) {
    var program_text;
    ...
    switch (operation) {
        ...
        case "lt":
            ...
            break;
        default:
            return panic("error number compare operation!");
    }
    program_text = `
